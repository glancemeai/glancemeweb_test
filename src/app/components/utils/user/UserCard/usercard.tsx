    ...
    return (
        <div className={styles.mainUserCardTwo}>
            ...
            <div className={styles.mainUserCardTwoDetails}>
                <Link href={`/tc/${props?.data?.user_id}`} passHref><p>{props?.data?.full_name ? props?.data?.full_name : props?.data?.first_name ? props?.data?.first_name : "Roten.x user"}</p></Link>
                <span>{props?.data?.friends ? (`${props?.data?.friends} connection `) : ""} <br />{props?.data?.location_name ? (`from ${props?.data?.location_name}`) : props?.data?.company_name ? props?.data?.company_name : "Roten.x"}</span>
            </div>
